...
const ResponseComprehension = ({ q, qi, r}) => {
    ...
    <div className='flex flex-col w-full items-center justify-center mb-2'>
      <div className='flex flex-col w-full pl-4 pr-6 items-center justify-center'>
        <div className='w-full pb-2'>
            <p className='w-full py-1 rounded-sm text-xs font-bold focus:outline-0 focus:shadow'>{q.description}</p>
        </div>
        <div className='w-full pb-2'>
            <p className='w-full py-1 rounded-sm text-xs focus:outline-0 focus:shadow'>{q.paragraph}</p>
        </div>
      </div>
      <div className='w-full pl-3 pr-5 mt-2'>
        {q && q.subQuestions && q.subQuestions.map((s,si)=>(
                  ...
                  <div className='w-full pb-2'>
                    {s.options.map((z,zi)=>(
                      <div className='flex flex-row items-center justify-start m-2' key={zi}>
                        <input type='radio' mid={si} name={si} id={zi} checked={r[si]} className='mr-2 ml-2'/>
...
export default ResponseComprehension